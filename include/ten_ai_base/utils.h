...
// 2. Add include dirs in BUILD.gn:
//      "//ten_packages/system/ten_ai_base/include"
//      "//.ten/app/ten_packages/system/ten_ai_base/include"
// 3. #include "ten_ai_base/utils.h"

namespace ten_ai_base {

inline const std::vector<std::string> &DefaultHeaderKeys() {
...
inline const std::vector<std::string> &DefaultJSONKeys() {
  static const std::vector<std::string> keys = {
...
inline const std::vector<std::string> &DefaultURLKeys() {
  static const std::vector<std::string> keys = {
      "sign", "signature",
      "accesskey",  "apikey",       "appkey",       "authorization",
      "key",        "password",     "secret",       "secretid",
...
inline size_t UTF8CodePointCount(const std::string &value) {
  size_t count = 0;
...
inline size_t UTF8ByteOffsetForCodePoint(const std::string &value,
...
inline std::string SHA256FingerprintPrefix(const std::string &value) {
  ...
  for (size_t i = 0; i < 4; ++i) {
    oss << std::setw(2) << static_cast<unsigned>((hash[0] >> ((3 - i) * 8)) & 0xFF);
  }
  return oss.str();
}
...
inline std::unordered_set<std::string> NormalizedKeySet(
...
inline bool IsSensitiveKey(const std::string &key,
...
inline std::string QueryUnescape(const std::string &input) {
  std::string output;
...
inline const std::vector<std::string> &EffectiveKeys(
    const std::vector<std::string> *keys,
...
inline std::string MaskSecret(const std::string &value) {
  if (value.empty()) {
    return value;
  }

  const size_t code_points = detail::UTF8CodePointCount(value);
  ...
  const size_t prefix_end =
      detail::UTF8ByteOffsetForCodePoint(value, step);
  const size_t suffix_start =
      detail::UTF8ByteOffsetForCodePoint(value, code_points - step);
  return value.substr(0, prefix_end) + "..." + value.substr(suffix_start) +
         "#" + detail::SHA256FingerprintPrefix(value);
}

namespace detail {

inline nlohmann::json RedactValue(const nlohmann::json &value) {
  if (value.is_null()) {
    return value;
  }
  if (value.is_string()) {
    return MaskSecret(value.get<std::string>());
  }
  if (value.is_array() || value.is_object()) {
    return MaskSecret(value.dump());
  }
  return MaskSecret(value.dump());
}

inline nlohmann::json RedactJSONValue(
...
inline std::string Encrypt(const std::string &value) { return MaskSecret(value); }

inline std::map<std::string, std::string> RedactHeaders(
...
inline std::string RedactURL(const std::string &raw_url,
  ...
  const auto normalized_keys = detail::NormalizedKeySet(url_keys);
  ...
  for (auto &pair : pairs) {
    ...
    if (!detail::IsSensitiveKey(detail::QueryUnescape(key), normalized_keys)) {
      continue;
    }
...
inline nlohmann::json RedactJSON(const nlohmann::json &value,
                                 const std::vector<std::string> &json_keys) {
  return detail::RedactJSONValue(value,
                                 detail::NormalizedKeySet(json_keys));
}
...
inline std::string RedactJSONDump(const nlohmann::json &value,
                                  const std::vector<std::string> &json_keys) {
